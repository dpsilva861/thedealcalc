...
export function SyndicationAcquisitionStep() {
  const { inputs, updateAcquisition } = useSyndication();
    ...
    <div className="space-y-6">
      <div className="bg-card rounded-xl p-6 border border-border shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <InputField label="Purchase Price" tooltip="Total acquisition price" value={acquisition.purchase_price} onChange={(v) => updateAcquisition({ purchase_price: v })} prefix="$" placeholder="1000000" />
          <InputField label="Closing Costs" tooltip="As % of purchase price" value={acquisition.closing_costs_pct * 100} onChange={(v) => updateAcquisition({ closing_costs_pct: v / 100 })} suffix="%" placeholder="2" min={0} max={20} />
          <InputField label="CapEx Budget" tooltip="Total renovation/improvement budget" value={acquisition.capex_budget_total} onChange={(v) => updateAcquisition({ capex_budget_total: v })} prefix="$" placeholder="50000" />
